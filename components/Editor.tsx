...
import { minutesFor, wordCount } from '@/lib/notes';
      ...
      <div className="page-head" style={{ marginBottom: 0 }}>
        <div>
          <p className="standfirst">
            {isNew ? 'A new note' : 'Editing'} · {words.toLocaleString()} {words === 1 ? 'word' : 'words'}
            {words > 0 && <> · {minutesFor(words)} min read</>}
          </p>
          <h1 className="screen-title">{isNew && !draft.trim() ? 'Your reading desk.' : title}</h1>
        </div>
